...
description:
  Python backend tests, Odoo test helpers, tags, `Form`, `HttpCase`, tours, and
  query-count assertions for catching regressions before they ship.
---

# Backend Testing and Tours

Use this file for the Odoo-specific pieces that matter in backend tests: class choice,
`Form`, tags, tours, and query budgets.
...
When business logic depends on defaults, onchanges, or relational widget-like semantics,
`Form` is usually better than calling `create()` with a giant dict.
...
This reference keeps only the backend and integration angle. For Hoot, web test helpers,
or mock-server specifics, load the `odoo-19-javascript-testing` skill instead.
